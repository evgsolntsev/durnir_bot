...
func TestLoot(t *testing.T) {
	...
	t.Run("Monsters win", func(t *testing.T) {
		defer func() {
			playerDAO.RemoveAll(ctx)
			fighterDAO.RemoveAll(ctx)
			fightDAO.RemoveAll(ctx)
		...
		fighters := []fighter.Fighter{{
			...
			Name:     "Монстр Ундо",
			Health:   100,
			Parts:    []fighter.Part{fighter.PartBeak},
			Fraction: fighter.FractionPlayers,
		}, {
			ID:       f4,
			Name:     "Монстр Сарасти",
		...
		fight.Fighters[2].Health = 0
		fight.Fighters[3].Health = 0
		...
		newUndo, err := playerDAO.FindOne(ctx, players[0].ID)
		require.NoError(t, err)
		require.Equal(t, 5050, newUndo.Gold)
		require.Equal(t, []fighter.Part{fighter.PartHand}, newUndo.Parts)

		newSaratie, err := playerDAO.FindOne(ctx, players[1].ID)
		require.NoError(t, err)
		require.Equal(t, 2600, newSaratie.Gold)
		require.Equal(t, []fighter.Part{fighter.PartWing}, newSaratie.Parts)
		...
		expectedMessage := "Ундо получает 50 золота за проданные части убитого монстра.\nСарасти получает 2500 золота за проданные части убитого монстра."
		expectedMessagesMap := make(map[idtype.Player]string)
		for _, pID := range []idtype.Player{newUndo.ID, newSaratie.ID} {
			expectedMessagesMap[pID] = expectedMessage
		}
