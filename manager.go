...
func (m *ItemsManager) init() error {
	...
	resp, err := srv.Spreadsheets.Values.Get(Config.Spreadsheet, "Sheet1!A2:N5").Do()
	if err != nil {
		return err
	}
	...
	for _, row := range resp.Values {
		...
		m.items[id] = make([]string, 0, 0)
		for i, cell := range row {
			if i < 3 {
				continue
			}
			m.items[id] = append(m.items[id], cell.(string))
		}
	}
